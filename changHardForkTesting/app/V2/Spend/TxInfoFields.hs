...
module V2.Spend.TxInfoFields where

import PlutusCore.Core (plcVersion100)
import PlutusLedgerApi.V1.Interval (after, before)
import PlutusLedgerApi.V1.Value (flattenValue, lovelaceValueOf)
...
mkValidator :: TxInfoFields -> () -> () -> [TxInInfo] -> [TxOut] -> [TxInInfo] -> Value -> POSIXTimeRange -> [PubKeyHash] -> Value -> Bool
mkValidator fields dat red inputs outputs refInputs mint range signatories fee =
  ...
  where
    ...
    hasFee = (lovelaceValueOf fee) < 1_000_000
